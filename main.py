...
def check_llm_configuration():
    """If live backend was chosen with no usable key, show the error for 2
    seconds and fall back to demo (mock) mode instead of crashing."""
    import config
    if config.LLM_BACKEND == "anthropic":
        api_key = os.environ.get("ANTHROPIC_API_KEY")
        if not api_key:
            console.print(
                "[bold red]⚠️ ERROR: Live mode selected but no API key found.[/bold red]\n"
                "[yellow]  No API key — falling back to demo (mock) mode.[/yellow]"
            )
            time.sleep(2)
...
def test_api_key(api_key: str) -> tuple[bool, str]:
    """Make a tiny real API call to prove the key works before committing to
    live mode. Returns (ok, message)."""
    try:
        import anthropic
        import config
        client = anthropic.Anthropic(api_key=api_key)
        client.messages.create(
            model=config.LLM_MODEL, max_tokens=8,
            messages=[{"role": "user", "content": "ping"}],
        )
        return True, "Key works — Anthropic responded."
    except Exception as exc:
        return False, f"API test failed: {exc}"


def _saved_key_path() -> str:
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), ".anthropic_key")


def load_saved_api_key() -> str | None:
    try:
        with open(_saved_key_path()) as f:
            key = f.read().strip()
        return key or None
    except OSError:
        return None
...
def save_api_key(api_key: str) -> None:
    with open(_saved_key_path(), "w") as f:
        f.write(api_key.strip() + "\n")
    os.chmod(_saved_key_path(), 0o600)


def startup_menu() -> int:
    """Ask live vs demo. Returns the run mode: 1=demo, 2=live.

    Key priority: ANTHROPIC_API_KEY env var → saved .anthropic_key → type it in.
    A typed key is tested with one tiny API call; on failure we show the error
    for 2 seconds and fall back to demo mode.
    """
    import config
    from rich.prompt import Confirm, Prompt

    console.print(Panel.fit(
        "[bold]Choose how to run Guardian CRM[/bold]\n"
        "[cyan]1)[/cyan] Live mode  — real Anthropic API (needs your API key)\n"
        "[cyan]2)[/cyan] Demo mode  — no key needed, mock LLM, continuous patient processing",
        border_style="bold blue",
    ))
    choice = Prompt.ask("Choose", choices=["1", "2"], default="2")
    if choice == "1":
        key = os.environ.get("ANTHROPIC_API_KEY") or load_saved_api_key()
        if key:
            source = "ANTHROPIC_API_KEY environment variable" if os.environ.get("ANTHROPIC_API_KEY") else f"saved key in {_saved_key_path()}"
            console.print(f"[green]✅ Using API key from {source}.[/green]")
        else:
            key = Prompt.ask("Enter your Anthropic API key", password=True).strip()
            if not key:
                console.print("[bold red]⚠️ ERROR: No API key entered.[/bold red]\n[yellow]  No API key — falling back to demo (mock) mode.[/yellow]")
                time.sleep(2)
                config.LLM_BACKEND = "mock"
                return 2
            if Confirm.ask("Save this key for next time?", default=True):
                save_api_key(key)
                console.print(f"[green]Key saved to {_saved_key_path()} (file permissions restricted).[/green]")
        console.print("[cyan]Testing the key with a tiny API call...[/cyan]")
        ok, message = test_api_key(key)
        if ok:
            console.print(f"[green]✅ {message}[/green]")
            os.environ["ANTHROPIC_API_KEY"] = key  # the anthropic SDK picks this up
            config.LLM_BACKEND = "anthropic"
            return 2
        console.print(f"[bold red]⚠️ ERROR: {message}[/bold red]\n[yellow]  Falling back to demo (mock) mode — continuous patient processing.[/yellow]")
        time.sleep(2)
        config.LLM_BACKEND = "mock"
        return 2
    config.LLM_BACKEND = "mock"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=[1, 2],
        default=None,
        help="Run mode: 1=demo (6-step fixed demo + generated patients), 2=live service (process patient bank). Omit to get the startup menu (demo or live with API key)."
    )
    parser.add_argument(
        "--count",
        type=int,
    ...
    # Startup: menu when no --mode flag was given, then always sanity-check the
    # LLM configuration (falls back to demo with a 2-second error if no key).
    if args.mode is None:
        args.mode = startup_menu()
    ...
    console.print(Panel.fit(
        ...
        f"[dim]LLM: {config.LLM_BACKEND} ({config.LLM_MODEL})[/dim]\n"
